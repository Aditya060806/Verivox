...
const start = async () => {
  try {
    ...
    // Connect to database (optional — server can still proxy to Python engine)
    try {
      await prisma.$connect();
      logger.info('Database connected');
    } catch (dbErr) {
      logger.warn(`Database connection failed — DB-dependent routes will be unavailable: ${dbErr.message}`);
    }
