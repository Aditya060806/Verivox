...
class LLMService {
  constructor() {
    if (config.openai.apiKey) {
      ...
      this.client = null;
      logger.warn('OPENAI_API_KEY not set — LLM features will return fallback responses');
    }
    this.model = config.openai.model;
  }

  /** Returns true when the OpenAI client is configured and usable. */
  get available() {
...
Return a JSON array with the following structure:
{
  ...
  async analyzeCredibility(content, title = '', source = '', claims = []) {
    if (!this.available) throw new Error('LLM not configured — set OPENAI_API_KEY');
...
${content.substring(0, 10000)}${claimsSummary}`;
  ...
  async verifyClaims(claims, retrievedContext = []) {
    if (!this.available || claims.length === 0) return [];
...
- UNVERIFIABLE: Cannot determine truth due to lack of sources`;
  ...
  async summarizeForFeed(content, title = '') {
    if (!this.available) return { summary: title || content.substring(0, 200), tags: [], category: 'OTHER' };
